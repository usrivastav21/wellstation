...
import { uploadCandidateVideoTrial } from "../../api/candidate/candidate.api";
import { uploadCandidateVideo } from "../../api/candidate/candidate.api";
...
export const FacialAnalysis = () => {
  ...
  const { currentDevice, stream, startStream, stopStream, getDevices } =
    useCamera();
  ...
  const cameraContainerHeight = useBreakpointValue(
    breakPointValues.cameraContainerHeight
  );
  ...
  }, [t, startStream, currentDevice?.deviceId]);
  ...
  useEffect(() => {
    const handleDeviceChange = async () => {
      addLog("Device change detected, re-enumerating devices");
      await getDevices();
    };
  ...
  const detectFaces = async (videoElement) => {
    try {
      ...
      // INFO: disabled temporarily due to client request
